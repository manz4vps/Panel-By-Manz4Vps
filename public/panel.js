...
function updateCommandPreview() { const engineEl = document.getElementById('set-engine'); const jarEl = document.getElementById('set-jar'); const previewEl = document.getElementById('cmd-preview'); if(!engineEl || !previewEl) return; const engine = engineEl.value || 'java'; const jar = jarEl ? (jarEl.value || 'server.jar') : 'server.jar'; const ramMB = window.ramLimitMB && window.ramLimitMB > 0 ? Math.round(window.ramLimitMB) : 2048; const ramStr = ramMB % 1024 === 0 ? `${ramMB / 1024}G` : `${ramMB}M`; let cmd = ''; if (engine === 'node') { cmd = `node ${jar}`; } else if (engine === 'python') { cmd = `python3 ${jar}`; } else { const flags = [ 'java', `-Xms125M`, `-Xmx${ramStr}`, `-XX:+UseG1GC`, `-XX:+ParallelRefProcEnabled`, `-XX:MaxGCPauseMillis=200`, `-XX:+UnlockExperimentalVMOptions`, `-XX:+DisableExplicitGC`, `-XX:+AlwaysPreTouch`, `-XX:G1NewSizePercent=30`, `-XX:G1MaxNewSizePercent=40`, `-XX:G1HeapRegionSize=8M`, `-XX:G1ReservePercent=20`, `-XX:G1HeapWastePercent=5`, `-XX:G1MixedGCCountTarget=4`, `-XX:InitiatingHeapOccupancyPercent=15`, `-XX:G1MixedGCLiveThresholdPercent=90`, `-XX:G1RSetUpdatingPauseTimePercent=5`, `-XX:SurvivorRatio=32`, `-XX:+PerfDisableSharedMem`, `-XX:MaxTenuringThreshold=1`, `-Dusing.aikars.flags=https://mcflags.emc.gs`, `-Daikars.new.flags=true`, `-jar ${jar}`, `--nogui` ]; cmd = flags.join(' '); } previewEl.innerText = cmd; }
